...
from utils.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
                           increment_path, non_max_suppression1, print_args, scale_boxes, strip_optimizer, xyxy2xywh)
...
def run(
        ...
        dist_thres=0.1,  # distractor threshold
        obj_thres=0.6,  # objectness threshold
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, (det, obj) in enumerate(zip(pred, obj_score)):
            ...
            detect_mask = det[:, 4] > conf_thres
            dist_mask = torch.logical_and(torch.logical_not(detect_mask), det[:, 4] > dist_thres)
            obj_mask = obj > obj_thres
            distractor_mask1 = torch.logical_and(dist_mask, det[:, 5] == len(names) - 1)
            distractor_mask2 = torch.logical_and(torch.logical_not(detect_mask), torch.logical_not(distractor_mask1))
            distractor_mask2 = torch.logical_and(distractor_mask2, obj_mask)
