...
def non_max_suppression1(
    ...
    mi = 5 + nc  # mask start index
    output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs
    objectness_score = [torch.zeros((0,), device=prediction.device)] * bs
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
        ...
        if not n:  # no boxes
            continue
        conf_mask = x[:, 4].argsort(descending=True)[:max_nms]
        ...
        output[xi] = x[i]
        objectness_score[xi] = o[i]
        if mps:
            output[xi] = output[xi].to(device)
            objectness_score[xi] = objectness_score[xi].to(device)
        if (time.time() - t) > time_limit:
            LOGGER.warning(f'WARNING ⚠️ NMS time limit {time_limit:.3f}s exceeded')
            break  # time limit exceeded

    return output, objectness_score
